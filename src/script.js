...
import { FileLoader, MeshStandardMaterial } from "three";
...
const wallHeight = walls.geometry.parameters.height;
...
const tick = () => {
  ...
  ghost1.position.x = Math.cos(ghost1Angle) * Math.sin(elapsedTime) * 10;
  ghost1.position.z = Math.sin(ghost1Angle) * Math.cos(elapsedTime) * 10;
  ghost1.position.y = Math.sin(elapsedTime * 2);

  const ghost2Angle = elapsedTime * 0.1;
  ghost2.position.x = Math.cos(ghost2Angle) * Math.cos(elapsedTime) * 8;
  ghost2.position.z = Math.sin(ghost2Angle) * 4;
  ghost2.position.y = Math.sin(elapsedTime);

  const ghost3Angle = elapsedTime * 0.1;
  ghost3.position.x = Math.sin(ghost3Angle) * Math.cos(elapsedTime) * 8;
  ghost3.position.z = Math.cos(ghost3Angle) * Math.sin(elapsedTime) * 8;
